...
//
//	base → TLS → Auth → SessionCookie → ExtraHeaders → OrgID → UserAgent → otelhttp
//
...
func BuildTransport(cfg *GrafanaConfig, base http.RoundTripper, opts ...TransportOption) (http.RoundTripper, error) {
	...
	if cfg.SessionCookieFile != "" {
		transport = NewDynamicCookieRoundTripper(transport, cfg.SessionCookieFile)
	} else if cfg.SessionCookie != "" {
		transport = NewCookieRoundTripper(transport, cfg.SessionCookie)
	}
...
var ExtractGrafanaInfoFromEnv server.StdioContextFunc = func(ctx context.Context) context.Context {
	...
	logger.Info("Using Grafana configuration", "url", parsedURL.Redacted(), "api_key_set", apiKey != "", "basic_auth_set", basicAuth != nil, "session_cookie_set", sessionCookie != "", "session_cookie_file", sessionCookieFile, "org_id", orgID, "extra_headers_count", len(extraHeaders))
	config.URL = u
			...
			if transportField.IsValid() && transportField.CanSet() {
				if _, ok := transportField.Interface().(http.RoundTripper); ok {
					...
					oboConfig := GrafanaConfig{
						...
						SessionCookieFile: config.SessionCookieFile,
					...
					logger.Debug("HTTP tracing, user agent tracking, timeout, and session cookie auth configured for Grafana client", "timeout", timeout, "session_cookie_set", config.SessionCookie != "", "session_cookie_file", config.SessionCookieFile)
				}
			}
		}
	}
